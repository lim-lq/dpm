...
export default {
  ...
  methods: {
    ...
    handleSubmit (e) {
      ...
      validateFields(validateFieldsKey, { force: true }, (err, values) => {
        if (!err) {
          ...
          var encrypted = encrypt.encrypt(values.password)
          console.log(encrypted)
          loginParams.password = encrypted
          Login(loginParams)
    ...
    requestFailed (err) {
      ...
      this.$notification['error']({
        message: '错误',
        description: ((err.response || err).data || err).info || '请求出现错误，请稍后再试',
        duration: 4
      })
    }
  }
